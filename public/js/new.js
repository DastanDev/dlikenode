	...
	steem.api.getDiscussionsByCreated(query, function (err, res) { res.forEach(($post, i) => {
		...
		if(metadata && metadata.community == "dlike"){
			...
			$(content).append('<div class="col-lg-4 col-md-6 postsMainDiv mainDiv'+ currentLikesDivElement +'" postLikes="0" postNumber="'+ currentPostNumber +'">\n' +
				...
				'\n' +
				'<div><a href="https://steemit.com/@' + $post.author + '" target="_blank"><img src="https://steemitimages.com/u/' + $post.author + '/avatar" alt="img" class="img-fluid my_img"></a></div>\n' +
				'\n' +
    		...
    		//user-pro status
			//$.ajax({type:"POST",url:"/helper/steem/getuserpoststatus.php",data:{author:author},dataType:"json",success:function(s){var t=(permlink+author).replace(".","");if("OK" 	==s.status){if("3"==s.setstatus){$(".userstatus_icon"+t).css({color:"red"});var e="PRO User"}$(".userstatus_icon"+t).hover(function(){toastr.success(e)})}else $(".userstatus_icon"+t).remove()}});
    		$.ajax({type: "POST",url: '/helper/steem/getuserpoststatus.php',data:{'author':author},dataType: 'json',success: function(response) {var mylabel = permlink +author;var newValue = mylabel.replace('.', '');if(response.status == "OK") {var all_status = response.setstatus;if(all_status == "3") {var colorset = 'red';$('.userstatus_icon' + newValue).css({"color": colorset});var erroset = "PRO User";}$('.userstatus_icon' + newValue).hover(function() {toastr.success(erroset);});	}else {$('.userstatus_icon' + newValue).remove();}}});
